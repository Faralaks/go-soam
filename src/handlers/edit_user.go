...
var Edit_user_data = http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
	...
	if err != nil {
		JsonMsg{Kind: FatalKind, Msg: "Произошла ошибка при смене данных пользователя | " + err.Error()}.Send(w)
		return
	}
	...
	if result.MatchedCount == 1 {
		...
		if err != nil {
			JsonMsg{Kind: FatalKind, Msg: "Произошла ошибка при удалении токенов пользователя | " + err.Error()}.Send(w)
			return
		}

		JsonMsg{Kind: ReloginKind}.Send(w)
		return
	}

	JsonMsg{Kind: BadUpdateKind}.Send(w)
